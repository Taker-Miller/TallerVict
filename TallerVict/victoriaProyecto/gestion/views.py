...
def inventario_jefe_view(request):
    ...
    return render(request, 'gestion/inventario_jefe.html', {'productos': productos, 'query': query})
...
def registro_ventas_jefe_view(request):
    # Obtener todas las ventas ordenadas por fecha
    ventas = Venta.objects.all().order_by('fecha')
    
    # Agrupar ventas por fecha y calcular el total por fecha
    ventas_por_fecha = {}
    total_por_fecha = {}
    total_general = 0
    ...
    for venta in ventas:
        if venta.fecha not in ventas_por_fecha:
            ...
            total_por_fecha[venta.fecha] = 0
        ventas_por_fecha[venta.fecha].append(venta)
        total_por_fecha[venta.fecha] += venta.total
        total_general += venta.total

    return render(request, 'gestion/ventas_jefe.html', {
        'ventas_por_fecha': ventas_por_fecha,
        'total_por_fecha': total_por_fecha,
...
def inventario_view(request):
    ...
    if query:
        ...
        productos_filtrados = Producto.objects.all()  # Mostrar todos los productos si no hay búsqueda
